...
布局文件缺失或损坏不会阻止框架启动。程序只在用户执行保存操作时写布局文件，不在退出时自动保存。`.qflayout` 中的 `QMainWindow::saveState()` 保存 Dock/标签拓扑，模块 JSON 的 `requestedVisible` 保存用户显示意图；`visible` 是旧程序兼容字段。旧文件只有 `visible` 时仍可读取，加载后应确认并重新保存。
