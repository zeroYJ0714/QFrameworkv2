...
## 主窗口 UI 外壳（2026-08-05）

`InProcessUi` 和 `ProcessUi` 的公开模块接口没有改变。它们仍由 `QDockWidget` 托管，
主窗口的菜单、无边框标题栏和窗口按钮属于 `MainWindow`/`WindowTitleBar`，模块不需要
知道 `HWND`、`WM_NCHITTEST` 或窗口 flags。模块只需提供原有 QWidget/窗口句柄和消息
生命周期；用户通过标题栏左侧“模块”菜单改变 Dock 的显示意图，布局保存仍由框架处理。

布局预设同样属于框架 UI 行为，不需要模块新增接口。框架从 `QFramework.ini` 读取连续的
`[Layout.n]`，在“布局”菜单直接创建 QAction；模块只需保持稳定的 Dock objectName，
Qt 的 `saveState()/restoreState()` 才能在预设之间恢复停靠关系。手动加载或另存为不会改变
当前预设，模块不应自行保存或修改 INI。

Dock 分隔条属于 Qt `QMainWindow::separator`，不是模块内部的 `QSplitter::handle`。因此
模块内部如果确实使用 `QSplitter`，仍可独立设置自己的 handle 样式，不会覆盖主窗口 Dock
之间的调整手柄。
