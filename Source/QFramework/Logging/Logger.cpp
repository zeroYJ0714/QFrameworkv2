// 文件职责：实现线程安全日志记录、文件滚动和 Qt 消息重定向。
// Logger 由框架拥有，互斥锁只保护文件句柄、级别和路径快照；业务线程可以直接调用日志
// 入口，但 QWidget、QSqlDatabase 和 ADB 资源不会在这里访问。关闭时先停止接收，再刷新
// 缓冲并关闭文件，避免后台消息写入已经释放的流。
#include "Logger.h"
